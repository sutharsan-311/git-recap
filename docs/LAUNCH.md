...
| --- | --- | --- |
| **X/Twitter** | Your own wrapped card + "I built Spotify Wrapped for git. 100% local, zero deps, `npx git-wrapped`" | Tue–Thu, 9am ET |
...
1. Every `wrapped.svg` embedded in a README is seen by every repo visitor → "make yours → npx git-wrapped" is printed on the card itself.
